...
# Define time points
  ...
  amt_load1 <- 3000  # loading dose CrCL = 100 mL/min (mg)
  amt_load2 <- 2750  # loading dose CrCL = 75 mL/min (mg) 
  amt_load3 <- 2500  # loading dose CrCL = 50 mL/min (mg) 
  amt_load4 <- 2000  # loading dose CrCL = 25 mL/min (mg)
...
# Add in individualised loading doses
  input_tb[with(input_tb, time == 0 & ID <= 1000), "amt"] <- amt_load1
  input_tb[with(input_tb, time == 0 & ID <= 2000 & ID >= 1001), "amt"] <- amt_load2
  input_tb[with(input_tb, time == 0 & ID <= 3000 & ID >= 2001), "amt"] <- amt_load3
  input_tb[with(input_tb, time == 0 & ID >= 3001), "amt"] <- amt_load4
...
# Save data as .RDS for later use
  ...
  readr::write_rds(output_tb, path = "output/vanc_induction_new.rds")
