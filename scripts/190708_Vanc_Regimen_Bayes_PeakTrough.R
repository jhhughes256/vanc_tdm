...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
  ...
  # Begin loop for successive dose adjustment
    ...
    # Determine previous dose and frequency
      ...
      sample_all <- c(sample_all, sample_time - ceiling(3*last_frq/4), sample_time)
    # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
    ...
    # Set initial dose and error estimates
      ...
      dose_amt <- exact_amt  # ceiling(exact_amt/250)*250
  ...
  readr::write_rds(output_tb, path = "output/vanc_regimen_bayes_asnoround.rds")
